...
Strataは、データベーススキーマをコードとして管理する**Infrastructure as Code (IaC)** CLIツールです。宣言的なYAMLファイルでスキーマを定義し、マイグレーションを自動生成・適用することで、複数環境にわたる安全で一貫性のあるスキーマ管理を実現します。
...
- **破壊的変更検出**: カラム削除・型変更等のリスクのある操作を事前に警告
- **JSON Schemaサポート**: IDE補完・バリデーション向けのスキーマ定義ファイル提供
...
change_note: 破壊的変更検出とJSON Schemaサポートを差別化要因に追加
