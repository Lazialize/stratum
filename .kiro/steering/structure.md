...
- **Key files**:
  ...
  - `commands/{command}.rs`: Individual command handlers
...
- **Key files**:
  ...
  - `config.rs`: Configuration models (`.stratum.yaml`)
...
- **Location**: `schema/` directory (configurable via `.stratum.yaml`)
...
- **`.stratum.yaml`**: User-facing config (generated by `stratum init`)
