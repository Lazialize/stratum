...
- [x] 1. DestructiveChangeReportモデルの実装
- [x] 1.1 (P) 破壊的変更レポートの構造体定義
...
- [x] 2. DestructiveChangeDetectorサービスの実装
- [x] 2.1 破壊的変更検出ロジックの実装
...
- [x] 3. CLI引数への--allow-destructiveフラグ追加
- [x] 3.1 (P) generateコマンドへのフラグ追加
  - clap derive macroで--allow-destructiveフラグを定義
  - デフォルト値をfalseに設定（デフォルト拒否の原則）
  - フラグの説明文を追加（破壊的変更の許可）
  - _Requirements: 3.1, 3.4_

- [x] 3.2 (P) applyコマンドへのフラグ追加
  - clap derive macroで--allow-destructiveフラグを定義
  - デフォルト値をfalseに設定
  - generateと同じ説明文を使用し一貫性を保つ
  - _Requirements: 3.2, 3.4_

- [x] 4. エラーメッセージフォーマッターの実装
- [x] 4.1 破壊的変更エラーの整形ロジック
...
- [x] 7. applyコマンドへの破壊的変更ガード統合
- [x] 7.1 メタデータからの破壊的変更判定と拒否
...
- [x] 7.2 --allow-destructive指定時の動作
  - 破壊的変更を含むマイグレーションを適用
  - 警告メッセージを表示
  - 古いマイグレーション（メタデータなし）の適用も許可
  - _Requirements: 3.2, 3.3, 3.5_

- [x] 7.3 dry-runモードでの破壊的変更表示
...
- [x] 8. MigrationPipelineのallow_destructive統合
- [x] 8.1 enum_recreate_allowedフラグの廃止
