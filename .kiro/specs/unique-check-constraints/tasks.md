...
- [x] 2. PostgreSQL・MySQL方言の制約操作SQL生成
- [x] 2.1 (P) PostgreSQL方言のUNIQUE・CHECK制約SQL生成
...
- [x] 2.2 (P) MySQL方言のUNIQUE・CHECK制約SQL生成
  - UNIQUE制約追加時に `ALTER TABLE ADD CONSTRAINT UNIQUE (columns)` を生成する（単一カラム・複合カラム両対応）
  - CHECK制約追加時に `ALTER TABLE ADD CONSTRAINT CHECK (expression)` を生成する
...
- [x] 4. マイグレーションパイプラインの制約処理統合
- [x] 4.1 UP方向の制約追加・削除処理
